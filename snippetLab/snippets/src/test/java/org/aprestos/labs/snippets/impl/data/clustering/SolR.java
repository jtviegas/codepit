...
public class SolR {
	...
	documents.add(createDoc("Monetary messages", 
	...
	documents.add(createDoc("England v Slovakia: 'Gareth Southgate's side pull through, but flaws persist'", 
	...
	documents.add(createDoc("Diego Simeone: Atletico Madrid manager signs two-year contract extension", 
	...
	documents.add(createDoc("Premier League 39th game plan 'not unnoticed by clubs' American owners'", 
	...
	documents.add(createDoc("Football clubs overspent by 30% on players - CIES Football Observatory study", 
	...
	documents.add(createDoc("World Cup 2018: Pat Nevin says Scotland v Slovakia game biggest for generation", 
	...
	String urlString = "http://localhost:8983/solr/eleanor";
    ...
    private static SolrInputDocument createDoc(String id, String content) {
    ...
    @Test
    public void testFullDoc() throws SolrServerException, IOException {
	
	SolrQuery query = new SolrQuery();
	
	query.set("fl", "id,content");
	query.set("q", String.format("content:%s",
		"Scotland's World Cup qualifier against Slovakia in October is the most important game in a generation, says former winger Pat Nevin.\n" + 
			"\n" + 
			"Following a 2-0 win over Malta on Monday, Gordon Strachan's side could finish second in Group F if they beat Slovakia then Slovenia, with eight runners-up reaching the play-offs.\n" + 
			"\n" + 
    ...
    @Test
    public void testTerms() throws SolrServerException, IOException {
	
	SolrQuery query = new SolrQuery();
	
	query.set("fl", "id,content");
	query.set("q", String.format("content:%s", "win over" ));
