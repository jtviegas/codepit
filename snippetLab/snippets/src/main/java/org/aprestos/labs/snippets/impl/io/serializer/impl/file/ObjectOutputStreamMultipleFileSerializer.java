...
public class ObjectOutputStreamMultipleFileSerializer implements Serializer<StringListOperation> {

	//ObjectOutputStreamMultipleFileSerializer-<operation id>.<timestamp>
	protected static final String NAME_FORMAT = "%s-%s.%s";
	protected static final String NAME_REGEXP = "ObjectOutputStreamMultipleFileSerializer-[0-9]+.[0-9]+";
	
	private File folder;
	
	public ObjectOutputStreamMultipleFileSerializer() {}
	...
	public void serialize(StringListOperation obj) throws SerializerException {
		...
		try {
			...
			File file = new File(folder.getAbsolutePath() + System.getProperty("file.separator") + 
					String.format(NAME_FORMAT, this.getClass().getSimpleName(), Long.toString(obj.getId()), Long.toString(System.currentTimeMillis())));
		...
		try {
			...
			for(File f:files){
				try {
					...
					obj = (StringListOperation)ois.readObject();
						result.add(obj);
				}
				finally { 
