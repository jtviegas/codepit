...
m <- mahalanobis(d[,2:7], center, cov_matrix)
#critical value
chis <- qchisq(0.95,df = 6)
selection <- m < chis
# we confirm line 1 is an outlier, 4, 28 and 60 also are flagged by 
# we now remove themmahalanobis, but with so few data we won't remove them at this stage
nrow(d)
d <- d[-c(1),]
nrow(d)
# 2nd iteration
...
# PC1 and PC2 which on the whole account for 99.74% of the variance, and their
# effect on the variance is somewhat leveled with PC1 accounting for 58.96% and PC2 for 40.78%
...
In the scores plot we can see that the higher ranked polymers are placed in the right upper quadrant associated 
with higher values of PC2 and lower values of PC1
and the lower ranked polymers are mostly located in the lower left quadrant
associated with higher values of PC1 and lower values of PC2."
...
"
Ranking          Mat_A             Mat_B            Mat_C            Mat_D            Mat_E            Mat_F       
Min.   : 1.00   Min.   :  2.225   Min.   : 3.999   Min.   : 3.018   Min.   : 3.001   Min.   : 3.999   Min.   :-9.595  
1st Qu.:15.75   1st Qu.: 13.421   1st Qu.: 8.505   1st Qu.: 7.850   1st Qu.: 6.527   1st Qu.:11.021   1st Qu.:18.314  
Median :30.50   Median : 65.266   Median :10.974   Median :11.026   Median :10.857   Median :20.666   Median :28.490  
Mean   :30.50   Mean   :128.821   Mean   :11.232   Mean   :10.885   Mean   :10.847   Mean   :20.689   Mean   :26.549  
...
for(i in seq(2, length(d), 1))
  boxplot(d[,i],xlab=names(d[i]), main=names(d[i]))
# we might have significant outliers in Mat_C
grubbs.test(d$Mat_C)
"
Grubbs test for one outlier
data:  d$Mat_C
...
Importance of components:
PC1    PC2     PC3       PC4       PC5       PC6
Standard deviation     1.8742 1.5408 0.33696 0.0005136 4.415e-05 1.487e-05
Proportion of Variance 0.5854 0.3957 0.01892 0.0000000 0.000e+00 0.000e+00
Cumulative Proportion  0.5854 0.9811 1.00000 1.0000000 1.000e+00 1.000e+00
"
# we will use the Kaiser Criterion to select the number of principal components, where
#  the sd is the squared root of the eigenvalue associated with the component, 
# the Kaiser Criterion chooses the eigenvalues higher that 1: ... so we choose:
# PC1 and PC2, also note that they are responsible repectively for 58.54% and 38.57% of the variance
...
"
"
again we have the same 15 NA's in F, as this is the same variables data. 
We'll adopt the irmi function approach, as in Part 1 there was not much difference between the two approaches.
...so we adopt the same analysis outcomes
"
d <- d2
d$Strength <- NULL
d$Warp <- NULL
...
dev.off()
dev.new()
strength_rankings <- d2[-c(1),]$Strength
rownames(d) <- strength_rankings
plot(pca$x)
text(pca$x,labels=strength_rankings, adj=c(0.3,-0.5), cex=0.7)
abline(h=0,v=0)
"
from the scores plot we can se thee that the observations with higher Strength ranking are 
positively correlated with PC1, and in a slight degree, negatively correlated with pc2. 
The higher ranked are as such distributed in the right quadrants with the extreme values in lower
right quadrant.
Consequently, the lower ranked observations are almost in its entirety negatively correlated with
pc1, with the correlation with pc2 not being that much clear in this case, so most of the 
lower ranked observations are placed along the pc2 axis towards the negative end of the pc axis
near the pc1 value -3 and around pc2=0
"
loadingplot(pca,scatter=T, labels = names(d), comps = c(1,2))
abline(h=0,v=0)

"
the loadings plot tells us that Mat_B and Mat_E are the most important variables
when trying to understand the Strength of the polymers, as are the most close to the 
higher ranked observations.
Mat_C and Mat_D are also related to lower Strength outcomes.
"

##########-------------d02 Warp resistance analysis----------################

warp_rankings <- d2[-c(1),]$Warp
...
"
the loadings plot tells us, coording to the score plot, 
that Mat_F and Mat_A are the most important variables
when trying to understand the warp resistance of the polymers.
So Mat_f seems to be a substancial part of lower ranked observations
where Mat_A is highly correlated with the higher rankings of warp resistance.
"
...
d0 <- as.data.frame(read.table("Polymer3.txt", header=T))
summary(d0)
"
