...
healthOutcomes <- function(datasets){
  ...
  mytheme <- theme(panel.background = element_rect(fill = "white", colour = "white", size = 2),
                   legend.key = element_rect(fill = "darkgrey")
                   , legend.title = element_text(colour = "steelblue", size = rel(2))
...
COUNTRIES <- c("DEU","RUS", "GBR", "USA", "PRT","IRL")
