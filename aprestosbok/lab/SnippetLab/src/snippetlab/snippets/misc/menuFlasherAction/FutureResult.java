...
public class FutureResult<T> 
{
  ...
  public Runnable setter(final Callable<T> function) {
  ...
  protected T doGet() throws InvocationTargetException {
    if (exception_ != null) 
      throw exception_;
    else
      return value_; 
  ...
  public synchronized Object timedGet(long msecs) 
    ...
    else if (waitTime <= 0) throw new TimeoutException();
    else {
      for (;;) {
        wait(waitTime);
        if (ready_) return doGet();
