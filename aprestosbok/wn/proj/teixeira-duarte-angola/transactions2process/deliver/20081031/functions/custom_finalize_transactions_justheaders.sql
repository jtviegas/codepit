CREATE OR REPLACE FUNCTION custom_finalize_transactions_justheaders(
	_temp_finalize_table varchar(50), _stored_table varchar(50), _transaction_first_number integer) 
returns integer
as 	
$BODY$	
declare
	_temp_finalize_table alias for $1;
...
for _r IN EXECUTE _sql LOOP
	...
	if _ok != 0 then
		raise exception '[>>] failed on finalizing headers of transaction -> %', _transaction_description;
		exit;
	end if;
...
_result := 0;
raise debug '[>>] <OUT> custom_finalize_transactions(%)', _result;
...
COMMENT ON FUNCTION custom_finalize_transactions_justheaders(_temp_finalize_table varchar(50), 
